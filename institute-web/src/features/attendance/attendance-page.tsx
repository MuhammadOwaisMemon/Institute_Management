...
export function AttendancePage() {
  ...
  <section className="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="grid gap-3 md:grid-cols-[1fr_220px]"><Field label="Batch"><select className="h-10 w-full rounded-lg border border-slate-200 px-3 text-sm" value={batchId} onChange={e=>setBatchId(e.target.value)}><option value="">Select batch</option>{batches.data?.data.map(b=><option key={b.id} value={b.id}>{b.name} - {b.course?.name}</option>)}</select></Field><Field label="Attendance date"><Input type="date" value={date} onChange={e=>setDate(e.target.value)}/></Field></div></section>
  ...
  <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><h2 className="text-base font-semibold text-slate-950">Monthly Report</h2><Field label="Month" className="sm:w-48"><Input type="month" value={month} onChange={e=>setMonth(e.target.value)}/></Field></div><div className="grid gap-3 md:grid-cols-3">{report.data?.map(r=><div key={r.student.id} className="rounded-lg border border-slate-100 bg-slate-50 p-4"><p className="font-semibold">{r.student.full_name}</p><p className="mt-1 text-sm text-slate-500">Present {r.present} | Absent {r.absent} | Leave {r.leave}</p><div className="mt-3"><StatusBadge status={r.percentage>=75?"active":"pending"} label={`${r.percentage}%`} /></div></div>)}</div></section></>;
}
...
function AttendanceEditor({ batchId, date, rows, isLoading, onSaved }: { batchId: string; date: string; rows?: AttendanceRow[]; isLoading: boolean; onSaved: () => void }) {
  ...
  const columns:Column<AttendanceRow>[]=[{key:"code",header:"Code",render:r=>r.student.student_code},{key:"name",header:"Student",render:r=>r.student.full_name},{key:"status",header:"Status",render:r=><div className="flex flex-wrap gap-2">{statuses.map(s=><button key={s} onClick={()=>setStatus(r.student.id,s)} className={`min-w-16 rounded-lg px-3 py-1.5 text-xs font-medium capitalize ${records[r.student.id]?.status===s?"bg-slate-950 text-white":"bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>{s}</button>)}</div>},{key:"remarks",header:"Remarks",render:r=><Input placeholder="Optional" value={records[r.student.id]?.remarks??""} onChange={e=>{setRecords(v=>({...v,[r.student.id]:{...v[r.student.id],remarks:e.target.value}}));setDirty(true)}}/>}];
  return <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">{dirty?<span className="rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-700">Unsaved changes</span>:null}<Button variant="outline" onClick={markAll} disabled={!rows}><Check className="h-4 w-4"/> Mark All Present</Button><Button onClick={()=>save.mutate()} disabled={!dirty||save.isPending}><Save className="h-4 w-4"/> Save</Button></div>{isLoading?<LoadingSkeleton className="h-64"/>:null}{rows&&rows.length>0?<DataTable columns={columns} data={rows.map((r,i)=>({...r,id:r.student.id+i}))}/>:null}{rows&&rows.length===0?<EmptyState icon={ClipboardCheck} title="No enrolled students" description="Students with active enrollments in this batch/date will appear here."/>:null}</section>;
}
