...
export function BatchesPage() {
  ...
  const batchesQuery = useQuery({
    queryKey: ["batches", debouncedSearch, status, courseId, teacherId],
    queryFn: () => getBatches({ search: debouncedSearch, status, course_id: courseId, teacher_id: teacherId }),
  });
    ...
    <>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-5 grid gap-3 lg:grid-cols-[1fr_180px_180px_180px]">
          <Field label="Search batches"><SearchInput value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search batches" /></Field>
          <Field label="Course"><select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm outline-none" value={courseId} onChange={(event) => setCourseId(event.target.value)}>
            <option value="">All courses</option>
            {coursesQuery.data?.data.map((course) => <option key={course.id} value={course.id}>{course.name}</option>)}
          </select></Field>
          <Field label="Teacher"><select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm outline-none" value={teacherId} onChange={(event) => setTeacherId(event.target.value)}>
            <option value="">All teachers</option>
            {teachersQuery.data?.data.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.full_name}</option>)}
          </select></Field>
          <Field label="Status"><select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm outline-none" value={status} onChange={(event) => setStatus(event.target.value)}>
