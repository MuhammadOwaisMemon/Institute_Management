      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {alerts.data && alerts.data.alerts.length > 0 ? (
          <div className="space-y-3">
            {alerts.data.alerts.map((alert) => <AlertRow key={alert.key} alert={alert} onRead={() => read.mutate(alert.key)} />)}
          </div>
        ) : null}
...
function AlertRow({ alert, onRead }: { alert: InternalAlert; onRead: () => void }) {
  return (
    <div className={cn("flex flex-col gap-3 rounded-lg border border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between", alert.is_read ? "bg-white" : "bg-slate-50")}>
      ...
      {!alert.is_read ? <Button variant="outline" size="sm" onClick={onRead}>Mark as read</Button> : <span className="text-xs font-medium text-slate-400">Read</span>}
    </div>
  );
}
