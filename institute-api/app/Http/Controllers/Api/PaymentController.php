...
class PaymentController extends ApiController
{
    ...
    public function store(StorePaymentRequest $request): JsonResponse
    {
        $payment = DB::transaction(function () use ($request) {
            ...
            $paid = (float) Payment::where('enrollment_id', $enrollment->id)->sum('amount');
            ...
            if ($installmentId) {
                $installment = FeeInstallment::where('institute_id', $request->user()->institute_id)->lockForUpdate()->findOrFail($installmentId);
            ...
            return Payment::create([...$request->validated(), 'institute_id' => $request->user()->institute_id, 'student_id' => $enrollment->student_id, 'receipt_number' => $this->nextReceipt($request->user()->institute_id), 'received_by' => $request->user()->id]);
        });
        return $this->success(new PaymentResource($payment->load(['student','enrollment.course','enrollment.batch','installment','receiver'])), 'Payment received.', 201);
    }
    ...
    public function show(Request $request, Payment $payment): JsonResponse
    {
        ...
        $paid = Payment::where('enrollment_id', $enrollment->id)->sum('amount');
    ...
    {
        $last = Payment::where('institute_id',$instituteId)->latest('id')->value('receipt_number');
